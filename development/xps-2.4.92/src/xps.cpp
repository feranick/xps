...
//**********************************************************************************
// to do: 
// line 905: The routine should be smart and understand the type of fit. Then it should use the proper function 
//  to produce the fit components (fitting curve already implemented.)
...
char Ytype[20];
char version[] = "2.4.92 (alpha) - 20100822";
...
double GL(double x, double F, double E, double m);
...
int extract(char *name, char *nameOUT, int infofile, int ext)
	...
	char Year[20], Month[20], Day[20], test[20], test2[20], bstype[20], fittype[20][20];
	...
	while(strcmp(test,"REGULAR")!=0)
		...
		if(strcmp(test,"Casa")==0)
			{casa=1;
			cout<<"\nWould you like to save the individual fit components (CasaXPS) (1: YES; 2: NO)\n";
			i=ReadKey();
			if(i==1)
				{casacomp=1;}	
			}	
	...
	while(!infile.eof())
			...
			{while(strcmp(test,"XPS")!=0 )
				...
				if(strcmp(test, "CASA")==0)
					...
					if(strcmp(test2, "region")==0)
						...
						{if(strcmp(test2, "comp")==0)
							{pea++;
							peak[g]=pea;
							infile>>test>>fittype[g]>>test;
							infile>>area[g];
							infile>>test>>test>>test>>test>>test;
							infile>>fwhm[g];
							infile>>test>>test>>test>>test>>test;
							infile>>position[g];
							}
		...
		// Here there should be some manipulation routine for fittype
		...
		if(casa==1 && g0!=g)
			{cout<<"\nRegion #"<<reg<<" boundaries: "<<KE-regend<<" - "<<KE-regstart<<" eV\n";
			cout<<" Background subtraction: "<<bstype<<endl<<endl;
				
			for (i=g0; i<g; i++)
				{position[i]=KE-position[i];
				cout << " Peak #"<<peak[i]<<" Fit type: " << fittype[i] << endl;			
				cout << "         Position: " << position[i] << endl;
				cout << "         FWHM: " << fwhm[i] << endl;
				cout << "         Area: " << area[i] << endl<<endl;
				
				if(casacomp ==1)
					{snprintf(nameout, CMAXI, "%s%s_reg%d_peak%d%s" , nameOUT, regfit, reg, peak[i], datextension);
					ofstream outfile(nameout);
					for(j=0; j<steps; j++)
						{if(xaxis==1 && bconv==1)
							{X[j]=	KE-regend+j*(regend-regstart)/steps;}
						else
							{X[j]=	regend+j*(regend-regstart)/steps;}
						
						if(Ytypef==1 && ctsconv==2)
							{}
						else
							{Y[j]=GL(X[j], fwhm[i], position[i], 30);}
		...
		if(infofile==1)
			...
			if(casa==1 && g0!=g)
				{outinfo<<"\n-----------\nCasaXPS:\n";
				outinfo<<"\nRegion #"<<reg<<" boundaries: "<<KE-regend<<" - "<<KE-regstart<<" eV\n";
				outinfo<<" Background subtraction: "<<bstype<<endl<<endl;
				for(i=g0; i<g; i++)
					{
					outinfo << " Peak #"<<peak[i]<<" Fit type: " << fittype[i] << endl;			
					outinfo << "         Position: " << position[i] << endl;
					outinfo << "         FWHM: " << fwhm[i] << endl;
					outinfo << "         Area: " << area[i] << endl<<endl;
				}
				outinfo<<"\n-----------";}
...
double GL(double x, double F, double E, double m)
	...
	double d;
	//cout<<F<<"\n"<<E<<"\n
	d=exp(-4*log(2)*(1-m/100)*pow((x-E)/F,2)) / (1+4*(m/100)*pow((x-E)/F,2));
	return d;
	}
