...
{	
	...
	#if     _MSC_VER
	ifstream	infile(ini, ios::nocreate);   //MSVC
	#else
			...
			if(flag==0)
				...
				if(vplot==1)
					...
					while(f1!=0)
						...
						sprintf(nametmp, "%s%s%d%s"  , nameOUT, regextV, f1, datextension);
						sprintf(nametmp2, "%s%s%d%s%s"  , nameOUT, regextV, f1, bsextension, datextension);
	...
	*bstype=ReadKey();
	...
	if(*bstype==1)
		...
		for (f=f1; f<=f2; f++)
			...
			sprintf(name, "%s%s%d%s" ,nameIN, regextV,f, datextension); 
			sprintf(nameOUT, "%s%s%d%s%s" ,nameIN, regextV,f, bsextension, datextension); 
	...
	{	
		...
		cin>> nameIN;	
		sprintf(name, "%s%s", nameIN, datextension);		
		sprintf(nameOUT, "%s%s%s" , nameIN, bsextension, datextension);		
...
if(type==3)
	...
	sprintf(name, "%s%s", nametmp, datextension);
	Fit(name);
	free(name);
	free(nametmp);	
	return main(0,0);
...
int DnD(char *name)
	{
	#if     _MSC_VER
	ifstream infile(name, ios::nocreate);		
	#else
...
int extract(char *name, char *nameOUT, int infofile, int ext)
	...
	if(ext==0)
		{sprintf(namein, "%s%s", name, ".vms");}
	else
		{sprintf(namein, "%s", name);}
 
	#if     _MSC_VER
	ifstream infile(namein, ios::nocreate);		
	#else
	...
	while(!infile.eof())
		...
		sprintf(nameout, "%s%s%d%s" , nameOUT, regextV, s, datextension);
		...
		if(infofile==1)
			...
			sprintf(nameinfo, "%s%s", nameOUT, infoextension);
...
int extractSPECTRA(char* nameIN, char * nameOUT, int infofile, int ext)
	...
	if(ext==0)
		{sprintf(namein, "%s%s", nameIN, ".1");}
	else
		{sprintf(namein, "%s", nameIN);}

	spflag =0;
	
	#if     _MSC_VER
	ifstream infile0(namein, ios::nocreate);		
	#else
	ifstream infile0(namein);		
	#endif
 	
	if(!infile0)
		{sprintf(namein, "%s%s", nameIN, spectraext);
		spflag=1;		
		}
	infile0.close();
	...
	{
		#if     _MSC_VER
		ifstream infile1(namein, ios::nocreate);		
		#else
	...
	overflow=0;
		...
		{	overflow++;
			...
			sprintf(nameout, "%s%s%d%s" , nameOUT, regextS, s, datextension);			
...
int savesheet(char* nameIN, int n, int d)
	...
	{	if(n>maxNregions)
		...
		if(d==1)		
			{sprintf(nameIN, "%s%s", nameIN, regextV);}
		else
			{sprintf(nameIN, "%s%s", nameIN, regextS);}
		
		sprintf(name, "%s", nameIN);
		
		for(i=1; i<=n; i++)
			{sprintf(name2, "%s%d_bs.dat", nameIN,i); 

			#if     _MSC_VER
			ifstream infile2(name2, ios::nocreate);		//MSVC
			#else
			...
			if(!infile2)
				{set.bs[i]=0;
				sprintf(name2, "%s%d.dat", nameIN, i); 				
				}
	
			#if     _MSC_VER
			ifstream infile(name2, ios::nocreate);		//MSVC
			#else
		...
		sprintf(name3, "%s.set.dat", nameIN);		
