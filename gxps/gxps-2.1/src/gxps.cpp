...
char Ytype[20];
char version[] = "2.1 (grace) - 20100817";
...
int savegrace(char* nameIN, int d);
...
//**************User interface******************************************************
	...
	cout<< "\nGXPS: Files Conversion, Background Subtraction and Multi-Peak Fitting \n";
	cout << "\n0) Exit";
...
// Menu option #9: "About"
	...
	if(type==9)
		{cout<<"\n GXPS: File Conversion, Background Subtraction and Milti-Peak Fitting\n v. "<<version;
		cout<<"\n\n Grace (version 5.1.x or higher) is required to plot ASCII files.\n";
...
// VMS and SPECTRA Converter
	...
	{	KEflag=0;
		...
		if(flag==0)		
			...
			if(dsheet==1)			
				{
				cout<<"\nSave global file in: (1: grace)   (2: spreadsheet)  (else: nothing) ";
				if(ReadKey()==1)   
					{if(type==1)			
						{savegrace(nameIN,1);}
					if(type==4)
...
if(type==3)
	...
	cout<<" Opening \""<<name<<"\" in Grace.\n";	
	cout<<" Please use the fitting functions in Grace.\n\n";
	//Fit(name);
...
void BackSub2(char* name, char* nameout)
	{	int *sbtype;
		...
		*sbtype=5;
		while(*sbtype==1 || *sbtype==2 || *sbtype==3 || *sbtype==4 || *sbtype==5 || *sbtype==8 || *sbtype==9 || *sbtype==0)
			{ 
...
{	if(vplot==1)	
		...
		pl++;
		if(pl>1)
			{GracePrintf("COPY S2 TO S1");
			GracePrintf("kill G0.S2");}
		GracePrintf("redraw");}
	else
		{}
	}
...
void PlotSettings()
	...
	if(xaxis==1 && bconv==2)		
		{GracePrintf("xaxis label \"Kinetic energy [eV]\"");}
	else
		{GracePrintf("xaxis label \"Binding energy [eV]\"");}
	if(ctsconv==2 && Ytypef==1)
		{GracePrintf("yaxis label \"counts\"");}
	else
		{GracePrintf("yaxis label \"counts/s\"");}
...
int savesheet(char* nameIN, int n, int d)
	...
	{	if(n>maxNregions)
		...
		cout<<" Saved "<<n<<" regions in spreadsheet: "<<name3<<"\n\n";
		free(name);
		free(name0);
		free(name2);
		free(name3);		
...
//*****************************************************************
// Save final regions in grace file (with background subtraction).
...
int savegrace(char* nameIN, int d)
	{	GraceRegisterErrorFunction(my_error_function);
		...
		char *name, *ext;		
		name=(char *)malloc(sizeof(char[CMAXI]));
		ext=(char *)malloc(sizeof(char[CMAXI]));
		int reg=1;
		
		if(d==1)		
			{snprintf(ext, CMAXI, "%s", regextV);}
		else
			{snprintf(ext, CMAXI, "%s", regextS);}

		while (reg<=maxNregions)		
			{snprintf(name, CMAXI, "%s%s%d_bs.dat", nameIN, ext, reg);
			ifstream infile(name);
			infile.close();
			if(!infile)
				{snprintf(name, CMAXI, "%s%s%d.dat", nameIN, ext, reg);
			...
			GracePrintf("read \"%s\"", name);
			GracePrintf("s%d legend \"Region: %d\"", reg-1, reg);
			GracePrintf("redraw");
			reg++;
		}
		GracePrintf("title \"%s.agr\"", nameIN);
		//GracePrintf("subtitle \"%d regions\"", reg-1);
		GracePrintf("legend ON");
		GracePrintf("saveall \"%s.agr\"", nameIN);
		cout<<"\n Saved "<<reg-1<<" regions in Grace file: "<<nameIN<<".agr\n\n";
		free(name);
		free(ext);	
	return 0;	
	}
